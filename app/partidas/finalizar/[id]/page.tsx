...
function ContadorGols({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-[#1E293B] font-extrabold text-[28px] w-[36px] text-center leading-none">
        {value}
      </span>
...
function SectionTitle({ children, badge }: { children: React.ReactNode; badge?: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between mb-3">
      <p className="text-[#374151] font-semibold text-[13px]">{children}</p>
      {badge}
    </div>
  );
}
...
export default function FinalizarPartidaPage() {
  ...
  function getNomeJogador(id: number) {
    return todosJogadores.find((j) => j.id === id)?.nome ?? "—";
  }

  function getTimeJogador(id: number) {
  ...
  function adicionarArtilheiro() {
    ...
    if (!jid) return;
    setArtilheiros((prev) => {
      const existente = prev.find((a) => a.jogadorId === jid);
      if (existente) {
        return prev.map((a) => a.jogadorId === jid ? { ...a, gols: a.gols + golsSelect } : a);
      }
  ...
  function removerArtilheiro(jid: number) {
    setArtilheiros((prev) => prev.filter((a) => a.jogadorId !== jid));
  }
  ...
  function adicionarCartao() {
    ...
    if (!jid) return;
    setCartoes((prev) => {
      // remove cartão anterior desse jogador (um ou outro)
      const semEste = prev.filter((c) => c.jogadorId !== jid);
      return [...semEste, { jogadorId: jid, tipo: tipoCartao }];
    });
    setSelectCartao("");
  }

  function removerCartao(jid: number) {
    setCartoes((prev) => prev.filter((c) => c.jogadorId !== jid));
  }
  ...
  // jogadores ainda não adicionados como artilheiros (para o select)
  const jogadoresDisponiveis = todosJogadores;

  // jogadores ainda não com cartão (para o select de cartões)
  const jogadoresSemCartao = todosJogadores.filter(
    (j) => !cartoes.find((c) => c.jogadorId === j.id)
  );
    ...
    <div className="min-h-screen bg-[#F1F3F7] px-4 py-4 md:px-5 md:py-5 lg:px-6 lg:py-6">
      <div className="max-w-[640px] mx-auto">
        ...
        <button onClick={() => router.push("/partidas")}
          className="flex items-center gap-2 text-[#374151] text-[14px] font-semibold mb-6 hover:text-[#4F6BED] transition-colors">
          <ArrowLeft size={16} />
          Voltar
        </button>
          ...
          <div className="p-5 flex flex-col gap-6">
            ...
            <div className="bg-[#F8FAFC] rounded-[14px] border border-[#E5E7EB] p-5">
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1 flex flex-col items-center gap-2">
                  ...
                  <span className="text-[#1E293B] font-bold text-[13px] text-center leading-tight">{partida.timeA}</span>
                </div>
                <div className="flex items-center gap-4">
                  <ContadorGols value={golsA} onChange={setGolsA} />
                  <span className="text-[#CBD5E1] font-bold text-[22px] mt-1">×</span>
                  <ContadorGols value={golsB} onChange={setGolsB} />
                </div>
                <div className="flex-1 flex flex-col items-center gap-2">
                  <EscudoTime nome={partida.timeB} escudo={partida.escudoB} size={48} />
                  <span className="text-[#1E293B] font-bold text-[13px] text-center leading-tight">{partida.timeB}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <select
                  value={selectArtilheiro}
                  onChange={(e) => setSelectArtilheiro(e.target.value)}
                ...
                <button type="button" onClick={adicionarArtilheiro}
                  disabled={!selectArtilheiro}
              ...
              {/* Lista de artilheiros adicionados */}
                  ...
                  {artilheiros.map((a) => (
                    <div key={a.jogadorId}
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-6 h-6 rounded-full bg-[#EEF2FF] flex items-center justify-center shrink-0">
                          <span className="text-[#4F6BED] text-[10px] font-bold">
                            {getNomeJogador(a.jogadorId).split(" ").map((w) => w[0]).join("").slice(0, 2)}
                          </span>
                        </div>
                        <div className="min-w-0">
                          <p className="text-[#1E293B] text-[12px] font-semibold truncate">{getNomeJogador(a.jogadorId)}</p>
                          <p className="text-[#94A3B8] text-[10px]">{getTimeJogador(a.jogadorId)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button type="button" onClick={() => removerArtilheiro(a.jogadorId)}
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <div className="flex items-center gap-1 bg-[#F8FAFC] border border-[#E5E7EB] rounded-[9px] p-1 h-[38px]">
                  ...
                  />
                  <button type="button"
                    onClick={() => setTipoCartao("vermelho")}
                    title="Vermelho"
                ...
                <button type="button" onClick={adicionarCartao}
                  disabled={!selectCartao}
                  className="w-[38px] h-[38px] rounded-[9px] bg-[#4F6BED] hover:bg-[#3D5BD9] disabled:bg-[#C7D2FE] disabled:cursor-not-allowed text-white flex items-center justify-center transition-colors shrink-0">
                  <Plus size={16} />
                </button>
              </div>

              {/* Lista de cartões adicionados */}
              {cartoes.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  {cartoes.map((c) => (
                    <div key={c.jogadorId}
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-6 h-6 rounded-full bg-[#EEF2FF] flex items-center justify-center shrink-0">
                          <span className="text-[#4F6BED] text-[10px] font-bold">
                            {getNomeJogador(c.jogadorId).split(" ").map((w) => w[0]).join("").slice(0, 2)}
                          </span>
                        </div>
                        <div className="min-w-0">
                          <p className="text-[#1E293B] text-[12px] font-semibold truncate">{getNomeJogador(c.jogadorId)}</p>
                          <p className="text-[#94A3B8] text-[10px]">{getTimeJogador(c.jogadorId)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {/* cartão visual */}
                        ...
                        <button type="button" onClick={() => removerCartao(c.jogadorId)}
                          className="w-[22px] h-[22px] rounded-full flex items-center justify-center text-[#94A3B8] hover:bg-[#FEE2E2] hover:text-[#EF4444] transition-colors ml-1">
                          <X size={12} />
                        </button>
                      </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Notas sobre a partida, incidentes, etc."
                rows={3}
